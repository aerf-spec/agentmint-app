...
describe("HomePage", () => {
  ...
  it("renders the procurement-focused homepage with the fallback intake email", async () => {
    ...
    expect(screen.getByText(/Your deal is in security review/i)).toBeInTheDocument();
    expect(
      screen.getByText(/We produce the missing evidence in 48 hours, attested under your name/i),
    ).toBeInTheDocument();
    ...
    expect(screen.getByText("48-HOUR DELIVERY · CONTINUOUS")).toBeInTheDocument();
    expect(screen.queryByText(/\$3,500 standard · 48-hour delivery/i)).toBeNull();
  });

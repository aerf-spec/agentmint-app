...
import { makeStressTools, STRESS_SPEC, stressSteps } from "../scenarios/coding-agent-stress.js";
...
function renderStep(r: StepResult): void {
  const note = r.note ?? "";
  if (r.outcome === "blocked") {
    console.log(`  ${icons.blocked} ${red(r.tool)}  ${red(bold("BLOCKED"))}  ${dim(note)}`);
  } else if (r.outcome === "warned") {
    console.log(`  ${icons.warned} ${yellow(r.tool)}  ${yellow("WARNED")}  ${dim(note)}`);
  } else {
    console.log(`  ${icons.allowed} ${fg(r.tool)}  ${dim("ok")}  ${dim(note)}`);
  }
}
...
async function scenarioA(): Promise<void> {
  ...
  console.log(boxLine(center(dim(`${stressSteps.length} tool calls · 5 rules · 1 breaker`))));
  console.log(boxBot());
  console.log("");

  // Drive the real engine through the shared stress scenario. Each line is
  // rendered from the actual per-step outcome — nothing here is scripted.
  const run = await runHarness(makeStressTools(), stressSteps, {
    spec: STRESS_SPEC,
    onStep: async (r) => { renderStep(r); await sleep(120); },
  });

  const { calls, allowed, blocked, warned } = run.counts;
  const caught = run.steps.filter((s) => s.outcome !== "allowed");
  const recovered = run.steps[run.steps.length - 1]?.outcome === "allowed";
  ...
  for (const s of caught) {
    const mark = s.outcome === "blocked" ? red("·") : yellow("·");
    console.log(boxLine(`    ${mark} ${fg(s.note ?? s.tool)}`));
  }
  console.log(boxBot());
  if (recovered) {
    console.log("");

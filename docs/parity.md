...
| `ConsoleOTelSink` | — | **omitted** — an OTel exporter would break the zero-runtime-dependency constraint; `ReceiptSink` is a two-line interface, so any exporter can be plugged in by the app |
...
| `AgentMint`, `Receipt`, `JtiStore` | — | **omitted** — the legacy pre-notary authorization-token API (JWT-style claims, single-use JTIs). Its job (pre-flight authorization with expiry and replay protection) is covered by plans (`signPlan`/`evaluatePolicy` with TTL) and `gate()` hash-chained approvals |
| `require_receipt`, `set_receipt`, `get_receipt`, `clear_receipt` | — | **omitted** — contextvar plumbing for the legacy decorator; `gate()` and `config.checkpoint` cover the pre-flight approval flow |
...
| RFC 3161 timestamping (`timestamp.py`) | — | **omitted** — issuance-time network calls to a TSA are out of scope for a zero-dependency SDK; the receipt shape reserves the post-issuance `timestamp` field (SPEC §11), so tokens can be attached by an outer layer without breaking signatures or chains |
